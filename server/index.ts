...
app.post('/api/videos', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error) {
      logger.error(`Error creating video(s): ${error.message}`);
      return res.status(500).json({ error: 'Failed to create video(s)' });
    }

    for (const item of data) {
      if (auto_schedule && target_account_ids.length > 0) {
        await schedulerService.scheduleForNextSlot(item.id);
      }
      videoPipeline.processVideo(item.id).catch(err => {
        logger.error(`Background processing error for ${item.id}: ${err}`);
      });
    }
